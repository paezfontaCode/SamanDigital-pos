/**
 * Página de listado de repuestos (inventario)
 */
...
export default async function RepuestosPage({ searchParams }: PageProps) {
  ...
  const [productsData, categories, suppliers, lowStockCount] = await Promise.all([
    ...
    getProductCategories('REPUESTO'),
    getSuppliers(),
    getLowStockCount('REPUESTO'),
  ]);
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Gestiona el inventario de repuestos para reparaciones
          </p>
        </div>
        <div className="flex gap-2">
          <NewProductModal type="REPUESTO" categories={categories} suppliers={suppliers} />
          <form action={async () => {
            'use server';
            const csv = await generateRestockCSV('REPUESTO');
            return new Response(csv, {
              headers: {
                'Content-Type': 'text/csv',
                'Content-Disposition': 'attachment; filename=reposicion-repuestos.csv',
              },
            });
          }}>
            <button
        ...
        <div className="mb-4 rounded-lg border-l-4 border-yellow-500 bg-yellow-50 p-4 dark:bg-yellow-950">
          <div className="flex items-center">
            ...
            <span className="font-medium text-yellow-800 dark:text-yellow-200">
              Hay {lowStockCount} {lowStockCount === 1 ? 'repuesto con' : 'repuestos con'} stock bajo
            </span>
          </div>
        </div>
      )}
      ...
      {/* Tabla de productos */}
      <Suspense fallback={<div className="text-center py-8">Cargando repuestos...</div>}>
        <ProductTable
